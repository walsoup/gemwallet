...
export default function AnalyticsScreen() {
  ...
  // ⚡ Bolt Optimization: Pre-compute current month's map to avoid redundant O(N) array filtering/iteration
  const { currentMonthSpentByCategory, totalIncome } = useMemo(() => {
    ...
    return { currentMonthSpentByCategory: map, totalIncome: income };
  }, [transactions, startOfMonth]);

  // Savings Logic
  const totalExpense = useMemo(() => {
    return Object.values(currentMonthSpentByCategory).reduce(
  ...
  const donutData = useMemo(() => {
    ...
    return Object.keys(currentMonthSpentByCategory)
      .map((catId, idx) => {
        const cat = categoryMap[catId];
        return {
          value: currentMonthSpentByCategory[catId] / 100,
  ...
  }, [currentMonthSpentByCategory, categoryMap, theme.colors]);
    ...
    transactions.forEach((tx) => {
      if (tx.type === "expense") {
        ...
        const match = monthsData.find(
          (m) =>
            m.year === txDate.getFullYear() && m.month === txDate.getMonth(),
        );
        if (match) {
  ...
  const moverPercentages = useMemo(() => {
    ...
    Object.keys(currentMonthSpentByCategory).forEach((categoryId) => {
      const current = currentMonthSpentByCategory[categoryId] ?? 0;
